    ...
    <div class="content-wrapper" :class="{ 'fade-out': isGenerating }">
      <div class="controls-row">
        <div class="control-group">
          <label for="noiseLevel" class="label-text">Noise Level: {{ noiseLevel }}</label>
          <input
            type="range"
            id="noiseLevel"
        ...
        <button class="generate-button" @click="generateNoises" :disabled="isLoading">
...
const generateNoises = async () => {
  ...
  try {
    const response = await fetch('https://api.codular.ru/api/v1/noises/generate', {
      method: 'POST',
      headers: {
    ...
    if (result.taskAlias) {
      ...
      error.value = 'No task alias received'
      isGenerating.value = false
      isLoading.value = false
    }
  } catch (err) {
